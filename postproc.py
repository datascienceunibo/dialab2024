...
##noout = non riportare l'output
##outsnip:<X>:<Y> = riporta solo le prime X e le ultime Y righe dell'output
##noerr = non riportare gli errori
...
def process_notebook(nb):
    ...
    for nnb in (nb_nosol, nb_sol):
        ...
        nnb.metadata.kernelspec = nb.metadata.kernelspec
    for cell in nb.cells:
        ...
        if cell.cell_type == "code" and cell.source.startswith(MAGIC_PREFIX):
            ...
            if magic[0] == "sol":
                ...
                write_to = [nb_nosol, nb_sol]
                cell.outputs = [
                    out for out in cell.outputs if out.output_type != "stream" or out.name != "stdout"
                ]
